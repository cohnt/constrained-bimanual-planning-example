...
IiwaBimanualPathCost::IiwaBimanualPathCost(
    int num_positions, int num_control_points,
    bool shoulder_up, bool elbow_up, bool wrist_up, double grasp_distance, bool square)
...
T IiwaBimanualPathCost::DoEvalGeneric(
    const Eigen::Ref<const Eigen::Matrix<T,Eigen::Dynamic,1>>& control_points_flat) const {

  // Row-major to match numpy convention.
  Eigen::Map<const Eigen::Matrix<T,Eigen::Dynamic,Eigen::Dynamic, Eigen::RowMajor>> control_points(
      control_points_flat.data(), num_positions_, num_control_points_);


  T total = T(0);
  for (int i = 1; i < num_control_points_; ++i) {
    Eigen::Matrix<T,Eigen::Dynamic,1> q0 =
        IiwaBimanualParameterization<T>(
            control_points.col(i-1), shoulder_up_, elbow_up_, wrist_up_, nullptr, grasp_distance_);
    Eigen::Matrix<T,Eigen::Dynamic,1> q1 =
        IiwaBimanualParameterization<T>(
            control_points.col(i), shoulder_up_, elbow_up_, wrist_up_, nullptr, grasp_distance_);
